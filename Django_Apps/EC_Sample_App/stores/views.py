...
class ProductListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if product_type_name:
            query = query.filter(product_type__name=product_type_name) # paramの左辺に注意(product_type__name)
        if product_name:
...
class ProductDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        """
        context = super().get_context_data(**kwargs)
        context['is_add'] = CartItems.objects.filter(
...
def add_product(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        if int(quantity) > product.stock:
            response = JsonResponse({'message': '在庫を超えています。'})
        ...
        cart = Carts.objects.get_or_create(user=request.user)
        if all([product_id, cart, quantity]):
            CartItems.objects.save_item(quantity=quantity, product_id=product_id, cart=cart[0])
...
class InputAddressView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        address = get_object_or_404(Addresses, user=self.request.user.id, pk=pk) if pk else address
...
class ConfirmOrderView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (not address) or (not total_price) or (not cart):
        ...
        # ログの出力
        application_logger.info(f'{cart}を削除しました。')
